...
### 12.4 左值引用(lvalue References) 和 右值引用(Rvalue References)
#### 12.4.1 左值引用是什么？
...
#### 12.5.2 使用 `move()`函数时需要注意什么？
&emsp;&emsp; 使用 `move()`函数时应该使用 `std::move()`，而不是使用'using std::move()`，这样可以避免潜在的名字冲突。

### 12.6 移动构造函数 和 移动赋值运算符
#### 12.6.1 移动构造函数(移动赋值运算符) 和 拷贝构造函数(拷贝赋值运算符) 有何区别？
**(1) 工作原理的区别**
...
#### 12.6.2 移动构造函数 需要完成哪些任务？
有下列任务：
> (1) 移动资源（一般通过 构造函数初始化列表完成）；
> (2) 则完成移动后，源对象必须不再指向被移动的资源。
> 
#### 12.6.3 为 `StrVec类` 定义移动构造函数
```cpp
...
StrVec::StrVec(const StrVec &&) noexcept: elements(s.elements),first_free(s.first_free), cap(s.cap)
{   
    s.elements = s.first_free = s.cap = nullptr;
}
```
#### 12.6.7 `noexcept`有什么好处？
&emsp;&emsp; 对于一个函数，标准库 默认该函数有可能抛出异常，并且为了处理这种可能性会做一些额外的工作。
&emsp;&emsp; 因此，通过 `noexcept` 告诉 标准库我们的函数不会抛出异常 将会提高性能。
#### 12.6.8 为什么在定义 移动构造函数 和 移动构造函数 时要使用 `noexcept` ？
(1) 提高性能
...
(2) 提供保障
&emsp;&emsp; 标准库容器对异常发生时其自身的行为提供保障，例如 vector 保证，如果调用 push_back 时发生异常，vector 自身不会改变。
对一个 vector 调用 push_back 可能会要求为 vector 重新分配内存，重新分配内存的过程中，vector 将从旧空间移动到新的内存中，如果使用的是移动构造函数并且在这个过程中发生了异常，不能保证vector的自身不变，但是如果使用的是拷贝构造函数就能保证异常发生时 vector 自身不变。
针对于以上机制，除非 vector 知道移动构造函数不会发生异常，否则重新分配内存的过程将调用拷贝构造函数而不是移动构造函数，为了显式地告知标准库移动构造函数可以安全使用，就需要通过移动构造函数和 noexcept 来做到这一点。

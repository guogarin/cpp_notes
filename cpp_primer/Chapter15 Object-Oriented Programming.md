- [第十五章 面向对象程序设计](#第十五章-面向对象程序设计)
  ...
  - [虚函数(virtual function)](#虚函数virtual-function)
    - [3.3 什么是虚函数(virtual function)](#33-什么是虚函数virtual-function)
    - [3.4 如何声明虚函数？](#34-如何声明虚函数)
    - [3.5 如何继承一个类？](#35-如何继承一个类)
    - [虚函数 和 非虚函数的成员函数 有什么区别？](#虚函数-和-非虚函数的成员函数-有什么区别)
    - [如果派生类没有覆盖它继承的虚函数 会发生什么？](#如果派生类没有覆盖它继承的虚函数-会发生什么)
      - [如何覆盖基类的虚函数？](#如何覆盖基类的虚函数)
...
&emsp;&emsp; 假设有以下继承结构：`class Drived : public Base`，然后有表达式`p->mem()`或`obj.mem()`，其中`mem()`是一个类成员函数，但具体是基类还是子类，现在可以不用管，而且我们也不用管`p`或者`obj`是指向哪个类。当程序的编译期，当编译器遇到表达式`p->mem()`或`obj.mem()`，执行以下步骤：
...
&emsp;&emsp; 不能，因为派生类中可能包含了基类不存在的部分，如果将基类绑定

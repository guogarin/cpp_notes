- [第十五章 面向对象程序设计](#第十五章-面向对象程序设计)
  ...
  - [33 派生类的友元 能否访问 基类的`protected`成员？为什么？](#33-派生类的友元-能否访问-基类的protected成员为什么)
    - [33.1 能不能？](#331-能不能)
    - [33.2 为什么？](#332-为什么)
  - [34.](#34)
...
&emsp;
&emsp;
## 33 派生类的友元 能否访问 基类的`protected`成员？为什么？
### 33.1 能不能？
&emsp;&emsp; 可以，但是只能通过派生类对象来访问基类的受保护成员：
```cpp
class Base {
protected:
...
```
### 33.2 为什么？
&emsp;&emsp; 假设派生类(及其友元)可以访问其基类对象的`protected`成员，则上面的`clobber(Base&)`函数则是正确的，这就意味着如果我们想绕开基类的`protected`限制，我们只需定义一个类似于`Sneaky类`的类即可，这样`protected`限制就失去了效果，想要阻止这样的用法，我们就必须规定 **派生类的成员和友元 只能访问 该派生类对象 中的基类部分 的`protected`成员，对于 普通基类对象中的成员 不具有特殊的访问权限。**

- [第十章 泛型算法](#第十章-泛型算法)
  ...
  - [list 和 forward_list 的特定容器算法](#list-和-forward_list-的特定容器算法)
      - [为什么通用版本的 sort算法不能用于 list 和 forward_list ？](#为什么通用版本的-sort算法不能用于-list-和-forward_list-)
      - [给它们提供的特殊版本的算法效率高还是通用算法的效率高呢？为什么？](#给它们提供的特殊版本的算法效率高还是通用算法的效率高呢为什么)
...
&emsp;&emsp;bind()函数接受一个可调用对象以及参数列表来生成一个新的可调用对象，从而达到更改参数顺序以及参数数量的目的
#### 适配器指的是啥？
...
```
**问题**
&emsp;&emsp; 错误，默认情况下，那些不是占位符的参数，bind()函数是传值，而流类型是不能拷贝的
**修改**
...
&emsp;&emsp;1st3 ： 1 2 3 4，因为inserter每次插入之后都会重新指向它原先指向的位置
...
&emsp;&emsp;
#### 如何使用流迭代器？
&emsp;&emsp;使用流迭代器时必须指定迭代器必须指定将要读写的对象的类型：
```cpp
...
| in1 == in2                    | in1 和 in2 必须读取相同类型。如果它们都是尾后迭代器或绑定相同的输入流，则相                        | 等。 |
| in1 != in2                    | 同上                                                                                               |
| *in                           | 返回从流中读取的值。                                                                               |
| in->mem                       | 同 (*in).mem                                                                                       |
...
#### 什么样的容器才有反向迭代器？
&emsp;&emsp;只有则迭代器支持++操作，也支持--操作的容器才有反向迭代器，forward_list的迭代器没有--操作，因此它没有反向迭代器。流也没有，因为流不支持--操作。
#### 如何获得反向迭代器？
...
vec.rbegin()指向的是容器的最后一个元素，而vec.end()指向的是尾后(最后一个元素的后面)；
vec.rend()指向的是容器的第一个元素前面的位置(该位置没有元素)，而vec.begin()指向的是容器的第一个元素
#### 如何将 反向迭代器 转为 普通迭代器？
...
#### 为什么通用版本的 sort算法不能用于 list 和 forward_list ？
&emsp;&emsp;因为通用版本的sort算法要求随机访问迭代器，而它俩没有随机访问迭代器，list提供的是双向对得起，而 forward_list是提供前向迭代器，因此需要为它们**分别提供**特殊的sort算法。
#### 给它们提供的特殊版本的算法效率高还是通用算法的效率高呢？为什么？
